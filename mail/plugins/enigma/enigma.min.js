...
rcube_webmail.prototype.enigma_key_create_save=function(){var a=[],b=$("#key-pass").val();var c=$("#key-pass-confirm").val();var d=$("#key-size").val();$('[name="identity[]"]:checked').each(function(){a.push({name:$(this).data("name")||"",email:$(this).data("email")})});if(!b||!c)return alert(this.get_label("enigma.formerror"));if(b!=c)return alert(this.get_label("enigma.passwordsdiffer"));if(!a.length)return alert(this.get_label("enigma.noidentselected"));if(window.openpgp&&window.crypto&&(window.crypto.getRandomValues||
window.crypto.subtle)){var e=this.set_busy(!0,"enigma.keygenerating");c={numBits:d,userIds:a,passphrase:b};openpgp.generateKey(c).then(function(a){rcmail.http_post("plugin.enigmakeys",{_a:"import",_keys:a.privateKeyArmored,_generated:1,_passwd:b,_keyid:a.key.primaryKey.fingerprint},e)},function(a){console.error(a);rcmail.set_busy(!1,null,e);rcmail.display_message(rcmail.get_label("enigma.keygenerateerror"),"error")})}else rcmail.display_message(rcmail.get_label("enigma.keygennosupport"),"error")};
